...
function basicAttackTimelineMarkup(operator) {
  ...
  const segmentMarkup = timeline.sequences.map((sequence) => {
    ...
    return `<div class="batk-segment" style="--segment-duration:${width}" title="${escapeHtml(`${sequence.label}: ${durationLabel}`)}">
      <div class="batk-segment-copy"><span class="batk-segment-index">${escapeHtml(sequence.label)}</span><strong>${escapeHtml(durationLabel)}</strong></div>
  ...
  const sequenceList = timeline.sequences.map((sequence) =>
    `<li><span>${escapeHtml(sequence.label)}</span><strong>${escapeHtml(formatDurationSeconds(sequence.duration))}</strong></li>`
  ).join("\n");
  ...
  return `<section class="panel batk-section" id="batk">
    ...
    <ol class="batk-sequence-list" aria-label="Basic Attack sequence durations">${sequenceList}</ol>
  </section>`;
}
